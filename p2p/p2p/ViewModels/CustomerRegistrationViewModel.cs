...
namespace p2p.ViewModels
{
    public class CustomerRegistrationViewModel : INotifyPropertyChanged
    {
        ...
        public CustomerRegistrationViewModel(IBackendProxy backendProxy, IBackendSessionManager backendSessionManager)
        {
             SelectedGender = -1;
        ...
        public async void OnSubmit()
        {
            ...
            else
            {
                ...
                string[] parms = { FirstName, LastName, SelectedGender.ToString(), BirthDate.ToString("yyyy-MM-ddT00:00:00"),
                                    PassportId, Phone1,"",Company, Address1,"","", Password, Email};
                var response = await _backendProxy.RegisterCustomerAsync(parms);
                if (response.Success)
                {
